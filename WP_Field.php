    ...
    if (!class_exists('WP_Field')):
        class WP_Field
        {
            ...
            private function checkbox( $field ) {
                printf(
                    '<label class="checkbox-label"><input type="checkbox" %s id="%s" name="%s" class="%s" %s > %s</label>',
                    $this->checked($field),
                    $field['id'],
                    $field['id'],
                    $field['class'],
            ...
            private function checked($field)
            {
                ...
                if (!empty($field['checked']) || 'yes' === $field['default'] || 'on' === $field['default']) { // true, 'checked'
                    $checked = 'checked';
                }

                $value = $field['val'] ?? $this->get_value($field['id']);
